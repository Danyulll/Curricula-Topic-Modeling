...
Perplexity is a measure of a model's ability to generalize to unseen data. It is the log-likelihood of a a test set of data and is given by Pleplé:
...
Unfortunately while the terms themselves are more informative they don't do well for interpreting topics.

## Collocations
Collocations are terms that have a higher probability of appearing together than they do individually. Rather than just have a model of singular terms or just have a model of bigrams we can have a model that incorporates the most common singular terms and bigrams.
...
BiGrams <- corpuz_tokzd %>% 
       quanteda::tokens_select(pattern = "^[A-Z]", 
                               ...
                               padding = TRUE) %>% 
       textstat_collocations(min_count = 4, tolower = FALSE,size=2)

BiGrams[order(BiGrams$count,decreasing = TRUE),]
```
This produces the most common collocations with statistical signifigance. Some of the terms are nonsensical like math math and are simply a product of us concatenating course descrptions. However we can examine the list and choose bigrams that we think ought to be included. Notable signifigant bigrams include:

The thing im doing is supported by this paper: https://people.eng.unimelb.edu.au/tbaldwin/pubs/tslp2013-topiccoll.pdf

It is suggested than you can go through your corpus and manually replace bigrams with a unigram representing the significant collocation to improve topic intelligibility. Therefore for all the significant bigrams I will do so except for the nonsensical ones such as math math or statistics data.

```{r}
library(stringr)
create_Bigram <- function(df,term1,term2,replacement){
...
# Combine collocations
df <- create_Bigram(df,"data","structures","data_structures")
df <- create_Bigram(df,"differential","equations","differential_equations")
df <- create_Bigram(df,"data","mining","data_mining")
df <- create_Bigram(df,"data","analysis","data_analysis")
df <- create_Bigram(df,"machine","learning","machine_learning")
df <- create_Bigram(df,"hypothesis","testing","hypothesis_testing")
df <- create_Bigram(df,"database","systems","database_systems")
df <- create_Bigram(df,"linear","regression","linear_regression")
df <- create_Bigram(df,"big","data","big_data")
df <- create_Bigram(df,"data","visualization","data_visualization")
df <- create_Bigram(df,"neural","networks","neural_networks")
df <- create_Bigram(df,"linear","models","linear_models")
df <- create_Bigram(df,"markov","chains","markov_chains")
df <- create_Bigram(df,"model","selection","model_selection")
df <- create_Bigram(df,"graph","theory","graph_theory")
df <- create_Bigram(df,"conditional","probability","conditional_probability")
df <- create_Bigram(df,"support","vector","support_vector")
df <- create_Bigram(df,"text","data","text_data")
```
```{r}
ds2 <- DataframeSource(df)
corpus2 <- Corpus(ds2)
dtm_col <- DocumentTermMatrix(corpus2)
...
dtm_col <-
  combineCols(c("model", "modeling", "modelling", "models"), dtm_col)

dtm_col <- combineCols(c("algorithm", "algorithms"), dtm_col)

dtm_col <- combineCols(c("learn", "learning"), dtm_col)

dtm_col <- combineCols(c("stat", "statistics","statistical"), dtm_col)

dtm_col <- combineCols(c("proof", "proofs"), dtm_col)

dtm_col <- combineCols(c("fields", "field"), dtm_col)
dtm_col <- combineCols(c("application","applications", "apply"), dtm_col)

dtm_col <- combineCols(c("logical", "logic"), dtm_col)

dtm_col <- combineCols(c("limit", "limits"), dtm_col)

dtm_col <- combineCols(c("distributions", "distribution"), dtm_col)

dtm_col <- combineCols(c("theorem", "theorems"), dtm_col)
...
```{r}
# Fit largest model
lda.out <- LDA(dtm_col, 8, method = "Gibbs",control = list(seed = 87460945))
...
temp_frequency <- as.matrix(dtm_col)
freq_matrix <- data.frame(ST = colnames(temp_frequency),
                          Freq = colSums(temp_frequency))
...
## Sampling multiple degree pathways
So far we have only been sampling 1 student's journey through a Data Science Curriculum. Perhaps if we sampled say 100 students from each program the most important courses would come to the surface with some added variance from electives.

First we create our corpus
```{r Sampling multiple degree pathways}
# Loading in data
...
Then we check for Bigrams to include
```{r}
corpusq <- quanteda::corpus(corpus)
...
# Combine collocations
df <- create_Bigram(df,"data","science","datascience")
df <- create_Bigram(df,"random","variables","randomvariables")
df <- create_Bigram(df,"data","structures","datastructures")
df <- create_Bigram(df,"differential","equations","differentialequations")
df <- create_Bigram(df,"data","mining","datamining")
df <- create_Bigram(df,"machine","learning","machinelearning")
df <- create_Bigram(df,"hypothesis","testing","hypothesistesting")
df <- create_Bigram(df,"big","data","bigdata")
df <- create_Bigram(df,"neural","networks","neuralnetworks")
df <- create_Bigram(df,"data","visualization","datavisualization")
df <- create_Bigram(df,"markov","chains","markovchains")
df <- create_Bigram(df,"graph","theory","graphtheory")
df <- create_Bigram(df,"linear","algebra","linearalgebra")
df <- create_Bigram(df,"confidence","intervals","confidenceintervals")
df <- create_Bigram(df,"time","series","timeseries")
df <- create_Bigram(df,"data","analytics","dataanalytics")
```
...
# Remove custom stop words
remove_words <-
  c("include","data","methods","topics","design","functions","emphasis","language","introduction","languages","performance","experience","course","science","techniques","variables","number","department","tools","fundamental","also","major","modern","issues","used","methods","using","case","architecture","covered","credit","basic","cosc","granted","use","solutions","will","students","fall","spring","important","one","considered","stacks","offers","types","may","held","former","honours","faculty","related","enter","review","enrolment","exercises","summer","need","offered","social","digital","terms","real","concepts","understanding","can","including","programs","program","recommended","examples","introduced","large","search","relations","key","etc","reasoning","intended","fws","general","restricted","version","two","comp","well","rich","intended","required","internet","recent","phys","sciences","covers","year","selected","renewal","explored","csch","principles","practice","development","studies","security","provides","advanced","instruction","discussed","processes","death","lower","high","crncr","taken","efficient","includes","core","retrieval","class","within","present","option","interested","together","session","week","new","order","tables","small","suitable","wide","without","good","introduces","assignments","current","thinking","completed","basics","essential","gain","effective","file","three","many","classes","extensive","tasks","work","meaningful","first","creating","elementary","image"
  )
corpus2 <- tm_map(corpus, function(x) {
  removeWords(x, remove_words)
})

dtm_total <- DocumentTermMatrix(corpus2)
```
...
dtm_total <-
  combineCols(c("model", "modeling", "modelling", "models"), dtm_total)

dtm_total <- combineCols(c("algorithm", "algorithms"), dtm_total)

dtm_total <- combineCols(c("learn", "learning"), dtm_total)

dtm_total <- combineCols(c("stat", "statistics","statistical"), dtm_total)

dtm_total <- combineCols(c("proof", "proofs"), dtm_total)

dtm_total <- combineCols(c("fields", "field"), dtm_total)
dtm_total <- combineCols(c("application","applications", "apply"), dtm_total)

dtm_total <- combineCols(c("logical", "logic"), dtm_total)

dtm_total <- combineCols(c("limit", "limits"), dtm_total)

dtm_total <- combineCols(c("distributions", "distribution"), dtm_total)
```


```{r}
# Fit largest model
lda.out <- LDA(dtm_total, 8, method = "Gibbs",control = list(seed = 87460945))
...
temp_frequency <- as.matrix(dtm_total)
freq_matrix <- data.frame(ST = colnames(temp_frequency),
                          Freq = colSums(temp_frequency))
...
## By course
maybe topic modeling where the courses are the document ids will give better results
```{r}
# Loading in data
load("../data/RObjects/degree_corpus_by_course.RData")
colnames(degree_corpus_by_course) <- c("doc_id", "text")
length(degree_corpus_by_course$doc_id) ==  length(unique(degree_corpus_by_course$doc_id))


df <- degree_corpus_by_course
#df <- degree_corpus_by_course[!duplicated(degree_corpus_by_course[,c('doc_id')]),]
#df <- df[-which(df$text==""),]
...
# Combine collocations
df <- create_Bigram(df,"data","science","data_science")
df <- create_Bigram(df,"random","variables","random_variables")
df <- create_Bigram(df,"data","structures","data_structures")
df <- create_Bigram(df,"differential","equations","differential_equations")
df <- create_Bigram(df,"data","mining","data_mining")
df <- create_Bigram(df,"machine","learning","machine_learning")
...
df <- create_Bigram(df,"markov","chains","markov_chains")
df <- create_Bigram(df,"graph","theory","graph_theory")
df <- create_Bigram(df,"linear","algebra","linear_algebra")
df <- create_Bigram(df,"confidence","intervals","confidence_intervals")
df <- create_Bigram(df,"time","series","time_series")
df <- create_Bigram(df,"data","analytics","data_analytics")
```
...
```{r}
dtm <- DocumentTermMatrix(corpus)
# dtm <- dtm[!which(slam::row_sums(dtm)==0),]
...
Pleplé, Q. (n.d.). Perplexity To Evaluate Topic Models. qpleple. Retrieved March 20, 2023, from 
